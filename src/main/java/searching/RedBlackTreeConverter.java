...
public class RedBlackTreeConverter {
    ...
    public static<Key extends Comparable<Key>> RBNode<Key> convert(TwoThreeNode<Key> twoThreeNode) {
         ...
         if (twoThreeNode.is2node()){
             RBNode<Key> left = convert(twoThreeNode.leftChild);
             RBNode<Key> right = convert(twoThreeNode.centerChild);
             int lenLeft = left == null ? 0 : left.size;
             int lenRight = right == null ? 0 : right.size;
             return new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, left, right,  Color.Black, lenLeft+lenRight+1);
         }else{
             RBNode<Key> left = convert(twoThreeNode.leftChild);
             RBNode<Key> middle = convert(twoThreeNode.centerChild);
             RBNode<Key> right = convert(twoThreeNode.rightChild);
             int lenLeft = left == null ? 0 : left.size;
             int lenMiddle = middle == null ? 0 : middle.size;
             int lenRight = right == null ? 0 : right.size;
             RBNode<Key> temp = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, left, middle,  Color.Red, lenLeft+lenMiddle+1);
             return new RBNode<>(twoThreeNode.rightKey, twoThreeNode.rightValue, temp, right, Color.Black, temp.size+lenRight+1);
         }
    }
    ...
    public static class RBNode<Key extends Comparable<Key>> {
        ...
        public RBNode(Key key, int value, RBNode<Key> left, RBNode<Key> right, Color color, int size) {
            this.key = key;
            this.value = value;
            leftChild = left;
            rightChild = right;
