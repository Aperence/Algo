...
public class PreorderToBST {
    ...
    */
    ...
    public Node preorderRead(int [] preOrderInput, int min, int max) {
        // !!!!!!! TODO !!!!!!!!
         if (count >= preOrderInput.length || preOrderInput[count] < min || preOrderInput[count] > max) return null;
         int number = count++;

         Node left = preorderRead(preOrderInput, min, preOrderInput[number]);
         Node right = preorderRead(preOrderInput, preOrderInput[number], max);
         return new Node(left,right,preOrderInput[number]);
    }
    ...
    public static void main(String[] args) {
        PreorderToBST bst = new PreorderToBST(new int[]{10,5,7,14,11});
