...
public class BST<Key extends Comparable<Key>, Value> implements Iterable<Key> {
    ...
    private Key higherKey(Node x, Key key, Key acc) {
         if (x == null) return acc;
         if (key.compareTo(x.key) >= 0) return higherKey(x.right, key, acc);
         if (acc == null || x.key.compareTo(acc) < 0) return higherKey(x.left, key, x.key);
         return higherKey(x.right, key, acc);
    }
    ...
    private class BSTIterator implements Iterator<Key> {
        ...
        public boolean hasNext() {
            if (n != nOp) throw new ConcurrentModificationException();
