...
public class BinarySearchTreeHigher<Key extends Comparable<Key>, Value> {
    ...
    public Key higherKey(Node n, Key v, Key ceil){
        if (n == null) return ceil;
        int cmp = v.compareTo(n.key);
        if (cmp < 0){
            if (ceil == null || ceil.compareTo(n.key) > 0) return higherKey(n.left, v, n.key);
            else return higherKey(n.left, v, ceil);
        }else return higherKey(n.right, v, ceil);
    }

    public Key higherKey(Key k){
        return higherKey(root, k, null);
    }
