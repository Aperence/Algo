...
public class MinMaxHeap<Key extends Comparable<Key>> {
    ...
    public Key max() {
        ...
        if (content[2].compareTo(content[3]) < 0) return content[3];
        return content[2];
    }
    ...
    public void swim(int position) {
        ...
        if (grandparent >= 1 && getNodeDepth(grandparent) % 2 == 0 && lessThan(content[position], content[grandparent])){
            swap(grandparent, position);
            swim(grandparent);
        }else if (parent >= 1  && getNodeDepth(parent) % 2 == 1 && higherThan(content[position], content[parent])){
            swap(parent, position);
            swim(parent);
        }else if (grandparent >=1 && getNodeDepth(grandparent) %2 == 1 && higherThan(content[position], content[grandparent])){
            swap(grandparent, position);
            swim(grandparent);
        }else if (parent >= 1  && getNodeDepth(parent) % 2 == 0 && lessThan(content[position], content[parent])){
            swap(parent, position);
            swim(parent);
        }
    }
