...
public class MinPQLinked<Key> {
    ...
    public Node getLastNode(Node root){
        root.size--;
        if (root.size == 1) {Node temp = root.left; root.left = null; return temp;}
        if (root.size == 2){ Node temp = root.right; root.right = null; return temp;}
        int nodes = root.size+1;
        int i = 1;
        while (nodes - i > 0){
            nodes = nodes - i;
            i*=2;
        }
        if (nodes > i/2)    return getLastNode(root.right);
        else                return getLastNode(root.left);
    }

    public void sink(Node root){
        Node min = root.left;
        if (min != null && root.right != null && comparator.compare(min.value, root.right.value) > 0) min = root.right;
        if (min != null && comparator.compare(root.value, min.value) > 0){
            exch(root, min);
            sink(min);
        }
    }

    public void sink2(Node root){
        if (root == null) return;
        root.size--;
        if (root.size==0){
    ...
    public Key delMin() {
        /*
         ...
         Key ret = root.value;
         if (root.size==1) {root=null; return ret;}
