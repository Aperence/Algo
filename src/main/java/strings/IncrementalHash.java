...
public class IncrementalHash {
    ...
    public int nextHash(char[] t, int previousHash, int from) {
        ...
        //
        /**
         * hash[t-1] = (t[from-1] * R^(M-1) + t[from] * R^(M-2) + ... + t[from+M-2]) % Q
         *
         * A = t[from-1] * R^(M-1)
         * B = t[from] * R^(M-2) + ... + t[from+M-2]
         *
         * hash[t-1] = (A + B) % Q
         * B % Q = (hash[t-1] + Q - A % Q) % Q
         *
         * hash[t] = (t[from] * R^(M-1) + t[from+1] * R^(M-2) + ... + t[from+M-1]) % Q
         * hash[t] = (B*R + t[from+M-1]) % Q
         * hash[t] = ((B % Q )*R + t[from+M-1] % Q) % Q
         *
         * hash[t] = (((hash[t-1] + Q - A % Q) % Q) * R + t[from+M-1] % Q) % Q
         */
        return (((previousHash + Q - (t[from-1]*RM) % Q) % Q)*R + t[from+M-1] % Q)%Q;
    }
    ...
    public static void main(String[] args) {
        ...
        int hashValue = (5*31 + 6)%10;
        System.out.println(hashValue);
        for (int i = 1; i < input.length-1; i++) {
            hashValue = hash.nextHash(input, hashValue, i);
            System.out.println(hashValue);
        }
    }
}