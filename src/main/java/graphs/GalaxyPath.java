...
public class GalaxyPath {
    ...
    public static int findPath(int [][] graph, int source, Set<Integer> destinations) {
        ...
        PriorityQueue<Integer> pq = new PriorityQueue<>((i, j) -> Integer.compare(L[i], L[j]) );
        pq.add(source);
        ...
        while(!pq.isEmpty()){
            ...
            marked[node] = true;
            if (destinations.contains(node)) return L[node];
            for (int i = 0; i < N; i++) {
                if (!marked[i] && L[node] < graph[node][i]){
                    pq.add(i);
                    if (L[i] > L[node] + 1) L[i] = L[node] + 1;
                }
            }
        }
        return -1;
