...
public class MineClimbing {
    ...
    private static class Node implements Comparable{
        ...
        public boolean isTarget(int x, int y){return this.x==x && this.y==y;}
    ...
    public static int revolve(int x, int max){
        if (x < 0) return max + x;
        return x % max;
    }


    public static int ind(int x, int y, int length){ return x*length + y;}
    /**
    ...
    public static int best_distance(int[][] map, int startX, int startY, int endX, int endY) {
        ...
        BitSet bt = new BitSet(map.length);
        while (!pq.isEmpty()){
            Node temp = pq.remove();
            if (bt.get(ind(temp.x, temp.y, M))) continue;
            bt.set(ind(temp.x, temp.y, M));

            if (temp.isTarget(endX, endY)){
                return temp.dist;
            }
            ...
            newX = revolve(temp.x+1, N);
            pq.add(new Node(newX, temp.y, temp.dist + Math.abs(map[temp.x][temp.y] - map[newX][temp.y])));

            newY = revolve(temp.y+1, M);
            pq.add(new Node(temp.x, newY, temp.dist + Math.abs(map[temp.x][temp.y] - map[temp.x][newY])));
