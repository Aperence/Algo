...
public class Trains {
    ...
    public static Map<String, Integer> reachableEarliest(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint) {
        ...
        hours.put(startPoint.station, startPoint.time);
        HashMap<String, LinkedList<StationTime>> lst = new HashMap<>();
        for (StationTime s : relations.keySet()){
            if (!lst.containsKey(s.station)) lst.put(s.station, new LinkedList<>());
            lst.get(s.station).add(s);
        }
        Stack<String> stack = new Stack<>();
        stack.push(startPoint.station);
        while (!stack.isEmpty()){
            String str = stack.pop();
            if (!lst.containsKey(str)) continue;
            for (StationTime from: lst.get(str)) {
                for (StationTime to : relations.get(from)){
                    if((!hours.containsKey(to.station)
                            || hours.get(to.station) > to.time)
                            && hours.get(from.station) < from.time) {
                        hours.put(to.station, to.time);
                        stack.push(to.station);
                    }
                }
            }
        }
